...
All 8 sensors appear under a single **NED Energy** device. Values are sourced from the `volume` field of the NED API, which is the production volume in **kWh** for the selected hourly interval. All applicable sensors have `state_class: measurement`, making them compatible with the HA statistics engine and long-term history graphs.

| Entity ID | Name | Unit | Description |
|---|---|---|---|
| `sensor.ned_total_production` | Total Production | kWh | Combined electricity output from all sources |
| `sensor.ned_solar_production` | Solar Production | kWh | Electricity generated by solar PV |
| `sensor.ned_wind_production` | Wind Production | kWh | Wind generation (type 1 — verify if offshore is included) |
| `sensor.ned_fossil_production` | Fossil Gas Production | kWh | Electricity generated from natural gas |
| `sensor.ned_consumption` | Electricity Consumption | kWh | National electricity demand |
| `sensor.ned_import` | Energy Import | kWh | Cross-border electricity imports |
| `sensor.ned_export` | Energy Export | kWh | Cross-border electricity exports |
...
For full API documentation — all parameters, data types, geographic points, rate limits, and timezone behaviour — see [docs/NED_API.md](docs/NED_API.md).
